//go:build amd64 && simba_syso && !cgo
// +build amd64,simba_syso,!cgo
...
TEXT ·sum_u8_raw(SB), NOSPLIT, $0-0
    MOVQ ptr+0(FP), DI
    MOVQ n+8(FP),  SI
    CALL sum_u8(SB)
    MOVL AX, ret+16(FP)
...
TEXT ·is_ascii_raw(SB), NOSPLIT, $0-0
    MOVQ ptr+0(FP), DI
    MOVQ n+8(FP),  SI
    CALL is_ascii(SB)
    MOVB AL, ret+16(FP)
...
TEXT ·validate_u8_lut_raw(SB), NOSPLIT, $0-0
    MOVQ ptr+0(FP), DI
    MOVQ n+8(FP),  SI
    MOVQ lut+16(FP), DX
...
TEXT ·map_u8_lut_raw(SB), NOSPLIT, $0-0
    MOVQ src+0(FP), DI
    MOVQ n+8(FP),  SI
    MOVQ dst+16(FP), DX
    MOVQ lut+24(FP), CX
